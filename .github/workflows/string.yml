name: String macros (persisted counter, scan all folders, zip top-level)
  ...
  workflow_dispatch:
    inputs:
      enable_chat:
        ...
        default: true
        type: boolean
      specific_folders_merge:
        description: 'Specif. folders merge'
        required: false
        default: false
        type: boolean
      versions:
    ...
    steps:
      ...
      - name: Execute string macro script
        run: |
          ...
          # Note: Whitelist not implemented for string_macros yet
          if [ "${{ inputs.specific_folders_merge }}" = "true" ]; then
            echo "⚠️ Whitelist not yet implemented for string_macros (will process all folders)"
          else
